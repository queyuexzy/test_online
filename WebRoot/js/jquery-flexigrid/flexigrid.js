...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			//rpOptions : [10],
			title : false, // 标题名称
			pagestat : '共 {total} 条数据,显示第 {from} 条到 {to} 条', // 显示当前页和总页面的样式
			...
			addData : function(data) { // parse data
				...
				if (p.total == 0) {
					...
					if (p.hideOnSubmit)
						$(g.block).remove();
					// 总数为0
					$(".pButton", this.pDiv).removeClass('button-disable');
					$(".pFirst.pButton", this.pDiv).addClass('button-disable');
					$(".pPrev.pButton", this.pDiv).addClass('button-disable');
					$(".pNext.pButton", this.pDiv).addClass('button-disable');
					$(".pLast.pButton", this.pDiv).addClass('button-disable');
					if (p.onSuccess)
						p.onSuccess(this, data);
					return false;
				}
				...
				if (p.dataType == 'json') {
					...
					$.each(data.rows, function(i, row) {
						...
						// add cell
						$('thead tr:first th', g.hDiv).each(function(i,th) {
...
//							display: table-cell;
						...
						// 添加多选
						if (p.showcheckbox) {
							$("#"+p.checkBoxId).removeAttr("checked");
							var cth = $('<th/>');
		...
		if (p.resizable && p.width != 'auto' && !p.novstripe) {
		...
		if ($('th', g.hDiv).length) {
			...
			if (p.showcheckbox) {
				$('tr', g.hDiv).each(function() {
					...
					var cthch = $('<input type="checkbox" id="'+p.checkBoxId+'"/>');
